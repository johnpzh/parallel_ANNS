//
// Created by Zhen Peng on 03/12/2021.
//
        ...
        double &avg_iters)
{
    ...
    for (unsigned i = 0; i < query_num; i++) {
        index.Search_neighbor_dist_to_parent(
                i,
                query_load + i * dim,
                data_load,
                K,
...
//    std::unordered_map<unsigned, double> recalls;
    ...
    {// average distance between neighbor and its parent
        for (idi s_i = 0; s_i < index.count_every_step_.size(); ++s_i) {
            if (0 == index.count_every_step_[s_i]) {
                continue;
            }
            printf("%u\t%f\n", s_i, index.profile_step_to_dist_to_parent_[s_i] / index.count_every_step_[s_i]);
        }
        index.profile_step_to_dist_to_parent_.clear();
        index.count_every_step_.clear();
    }
    runtime = diff.count();
