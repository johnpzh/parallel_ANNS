//
// Created by Zhen Peng on 06/28/2020.
//
...
//        printf("source_id: %u\n", engine.ep_);
    ...
    {// Add source to step 2 vertices, and add target to nearest neighbors
        for (idi v_i = 0; v_i < points_num; ++v_i) {
            if (!is_connected_to_nn[v_i]) {
                continue;
            }
            if (my_step[v_i] < 2) {
                connected_to_nn_edgelist[v_i].clear();
            }
            if (my_step[v_i] == 2) {
                connected_to_nn_edgelist[source_id].push_back(v_i);
            }
        }
        ...
        for (idi k_i = 0; k_i < K; ++k_i) {
            ...
            idi nn_id = true_nn_list[selected_query_id][k_i];
            connected_to_nn_edgelist[nn_id].push_back(target_id);
        }
...
//                continue;
//            }
//            fprintf(fout, "%u\n", true_nn_list[selected_query_id][k_i]);
//        }
    }
