...
#include "../core/Searching.202105281923.PSS_v5_LG_nn_distribution.h"
//#define DEBUG_PRINT
...
//                engine.prepare_init_ids(init_ids, L_local);
                ...
                for (unsigned q_i = 0; q_i < query_num; ++q_i) {
                    ...
                    std::set<PANNS::idi> nn_ids;
                    for (PANNS::idi v_id : set_K_list[q_i]) {
                        nn_ids.insert(v_id);
                    }
                    std::vector< std::set<PANNS::idi> > hit_iters_list(num_threads);
                    PANNS::idi iter_count = 0;
                    engine.para_search_get_nn_distribution(
                            q_i,
                            K,
                            L_master,
                            set_L,
                    ...
                    {// Print
                        for (int th_i = 0; th_i < num_threads; ++th_i) {
                            for (PANNS::idi iter : hit_iters_list[th_i]) {
                                printf("%d %u\n", th_i, iter);
                            }
                        }
                        printf("iter_count: %u\n", iter_count);
