//
// Created by Zhen Peng on 11/15/2021.
//
...
#include "../core/Searching.202111152031.PSS_v5.index_adaptive_merge_L_master.LG.h"
...
//        const unsigned subsearch_iterations,
        const unsigned index_threshold,
...
//#pragma omp parallel for
    for (unsigned q_i = 0; q_i < query_num; ++q_i) {
        ...
        engine.para_search_PSS_v5_index_adaptive_merge_LG(
                q_i,
                K,
                L_master,
                set_L,
...
//            "Usage: %s <data_file> <query_file> <nsg_path> <L_lower> <K> <result_file> <true_NN_file> <num_threads> <sub_iters> <L_upper> <P@100> [<P@100> ...]\n",
                ...
                "<L_master> <L_local_lower> <L_local_upper> <X> "
                "<P@100> [<P@100> ...]\n",
                argv[0]);
        exit(EXIT_FAILURE);
    }
...
//    omp_set_max_active_levels(2);
    ...
    const unsigned L_local_lower_origin = strtoull(argv[9], nullptr, 0);
    const unsigned L_local_upper_origin = strtoull(argv[10], nullptr, 0);
//    const unsigned X = strtoull(argv[11], nullptr, 0);
    const double index_ratio = strtod(argv[11], nullptr);
...
//        std::unordered_map<unsigned, double> recalls;
        unsigned L_upper = L_local_upper_origin;
        unsigned L_lower = L_local_lower_origin;
//        unsigned L_master = L_upper;
        unsigned L_local = L_upper;
//        unsigned subsearch_iterations = X;
        unsigned index_threshold = L_local * index_ratio;
        double runtime;
        uint64_t compt;
        double recall;
        double avg_merge;
...
//                L_master = (L_lower + L_upper) / 2;
                ...
                index_threshold = L_local * index_ratio;
            }
        }

        L_upper = L_local_upper_origin;
