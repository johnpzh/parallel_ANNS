//
// Created by Zhen Peng on 08/05/2021.
//
...
#include "../core/Searching.202107292234.PSS_v5.multi_queries.LG.h"
...
//    omp_set_nested(1); // Deprecated.
    omp_set_max_active_levels(2); // OpenMP nested parallelism
    int num_threads_outter;
    if (num_threads > 8) {
        engine.num_threads_inner_ = 8;
        num_threads_outter = num_threads / 8;
    } else {
        engine.num_threads_inner_ = num_threads;
        num_threads_outter = 1;
    }
//    engine.num_threads_inner_ = 8;
...
//                std::vector<PANNS::idi> local_queues_starts(num_threads);
                std::vector<PANNS::idi> local_queues_starts(engine.num_threads_inner_);
                for (int q_i = 0; q_i < engine.num_threads_inner_; ++q_i) {
                    local_queues_starts[q_i] = q_i * L_local;
                }
//                std::vector<PANNS::idi> top_m_candidates(num_threads);
                uint64_t cnt_dist_compt = 0;
                std::vector<double> outter_runtime(num_threads_outter, 0);
                std::vector<uint32_t> outter_counter(num_threads_outter, 0);
                auto s = std::chrono::high_resolution_clock::now();
//                engine.prepare_init_ids(init_ids, L_local);
                engine.prepare_init_ids(init_ids, L_master);
#pragma omp parallel for num_threads(num_threads_outter) reduction(+ : cnt_dist_compt)
                for (unsigned q_i = 0; q_i < query_num; ++q_i) {
                    double t_q = -PANNS::WallTimer::get_time_mark();
                    engine.para_search_PSS_v5_multi_queries_LG(
                            q_i,
                            K,
                            L_master,
//                            set_L,
...
//                            is_visited,
                            ...
                            cnt_dist_compt);
                    t_q += PANNS::WallTimer::get_time_mark();
                    int out_tid = omp_get_thread_num();
                    outter_runtime[out_tid] += t_q;
                    ++outter_counter[out_tid];
//                    ////////////////////////////
//                    //// CAREFUL!!!!
//                    if (q_i == 0) {
//                        break;
...
//                    ////////////////////////////
                ...
                double out_avg_latency = 0;
                {
                    // Outter avg time
                    for (int tid = 0; tid < num_threads_outter; ++tid) {
                        out_avg_latency += outter_runtime[tid] / outter_counter[tid] * 1000.0;
                    }
                    out_avg_latency /= num_threads_outter;
                }
                {// Basic output
                    printf(
                            "num_threads: %d "
...
//                            "t_p_seq(%%): %f ",
                            ...
                            diff.count() * 1000 / (query_num / num_threads_outter),
                            recalls[100],
                            recalls[1],
                            data_dimension * 4.0 * cnt_dist_compt / (1U << 30U) /
                            diff.count(),
                            data_dimension * (1.0 + 1.0 + 1.0) * cnt_dist_compt /
                            (1U << 30U) /
                            diff.count(),
                            L_local,
                            subsearch_iterations,
...
////                            engine.time_pick_ / diff.count() * 100.0);
                    // Outter total time
                    for (int tid = 0; tid < num_threads_outter; ++tid) {
                        printf("out_runtime[%d](s.): %f ", tid, outter_runtime[tid]);
                    }
                    // Outter avg time
                    for (int tid = 0; tid < num_threads_outter; ++tid) {
                        printf("out_latency[%d](ms.): %f ", tid, outter_runtime[tid] / outter_counter[tid] * 1000.0);
                    }
                    printf("\n");
                }
                cnt_dist_compt = 0;
