...
{
    fprintf(stderr,
            "Usage: %s <data_file> <query_file> <nsg_path> <L> <K> <result_file> <true_NN_file> <quota>\n",
            argv[0]);
}
...
//    unsigned subsearch_iterations = strtoull(argv[10], nullptr, 0);
    uint64_t computation_quota = strtoull(argv[8], nullptr, 0);
    engine.thread_computation_quota_ = computation_quota / query_num;
...
//#pragma omp parallel for
        for (unsigned q_i = 0; q_i < query_num; ++q_i) {
            engine.para_search_with_sequential_access(
                    q_i,
                    K,
                    L,
                    set_L,
