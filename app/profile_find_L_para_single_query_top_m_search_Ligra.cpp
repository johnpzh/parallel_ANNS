//
// Created by Zhen Peng on 11/23/2020.
//
...
#include "../core/Searching.202012191846.top_m_Ligra.h"

void do_ligra(
        char *ligra_filename, // adj file
        int argc,
        char *argv[])
{
    commandLine P(argc, argv, " please check the arguments.");
    bool compressed = false;
    bool symmetric = false;
    bool binary = false;
    bool mmap = false;
    graph<asymmetricVertex> G =
            readGraph<asymmetricVertex>(ligra_filename,compressed,symmetric,binary,mmap); //asymmetric graph
...
//                std::vector<uint8_t> is_visited(points_num, 0);
    ...
    for (unsigned q_i = 0; q_i < query_num; ++q_i) {
        engine.para_search_with_top_m_pure(
                M,
                q_i,
                K,
                L,
