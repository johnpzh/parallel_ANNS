...
#include "../core/Searching.202012122112.top_m_pure_large_graph.backtracking.h"
...
//                std::vector<uint8_t> is_visited(points_num, 0);
    ...
    for (unsigned q_i = 0; q_i < query_num; ++q_i) {
        engine.para_search_with_top_m_pure_large_graph_backtracking(
                M,
                q_i,
                K,
                L,
...
//                            "local_queue_length: %u "
                ...
                engine.count_backtracking_ * 1.0 / query_num);
        printf("\n");
    }
    runtime = diff.count();
    ...
    avg_backtracking = engine.count_backtracking_ * 1.0 / query_num;
    engine.count_backtracking_ = 0;
...
//        for (unsigned i = 0; i < query_num; i++) set_K_list[i].resize(K);
        ...
        while (L_lower <= L_upper) {
            ...
            if (recalls[100] < P_dest) {
                ...
                last_avg_backtracking = avg_backtracking;
            } else {
                break;
            }
            if (L_lower <= L_upper) {
        ...
        if (recalls[100] < P_dest && L < L_upper) {
            ...
            avg_backtracking = last_avg_backtracking;
        }
