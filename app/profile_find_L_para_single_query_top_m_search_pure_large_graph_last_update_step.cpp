...
#include "../core/Searching.202012041559.top_m_pure_large_graph.last_update_step.h"
...
//                std::vector<uint8_t> is_visited(points_num, 0);
    ...
    engine.record_last_update_iter_.resize(L, 0);
    auto s = std::chrono::high_resolution_clock::now();
    engine.prepare_init_ids(init_ids, L);
    for (unsigned q_i = 0; q_i < query_num; ++q_i) {
        engine.para_search_with_top_m_pure_large_graph(
                M,
                q_i,
                K,
                L,
...
//    std::unordered_map<unsigned, double> recalls;
    ...
    {// last update step
        for (unsigned e_i = 0; e_i < L; ++e_i) {
            engine.record_last_update_iter_[e_i] /= query_num;
        }
        for (unsigned e_i = 0; e_i < L; ++e_i) {
            printf("%u\t"
                   "%f\n",
                   e_i + 1,
                   engine.record_last_update_iter_[e_i]);
        }
    }
    {// Basic output
        printf(
