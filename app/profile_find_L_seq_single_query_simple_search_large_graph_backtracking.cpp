...
#include "../include/efanna2e/index_nsg.202012111616.backtracking.h"
        ...
        double &avg_backtracking)
{
    ...
    for (unsigned i = 0; i < query_num; i++) {
        index.Search_backtracking(query_load + i * dim, data_load, K, paras, res[i].data());
    }
...
//    std::unordered_map<unsigned, double> recalls;
    ...
    {// Basic output
        printf(
                ...
                index.count_backtracking_ * 1.0 / query_num);
        printf("\n");
    }
    runtime = diff.count();
    ...
    avg_backtracking = index.count_backtracking_ * 1.0 / query_num;
    index.count_backtracking_ = 0;
    set_K_list.swap(res);
...
//        L = (L_lower + L_upper) / 2;
            ...
            if (recalls[100] < P_dest) {
                ...
                last_avg_backtrack = avg_backtrack;
            } else {
                break;
            }
            if (L_lower <= L_upper) {
...
//        L_upper = strtoull(argv[8], nullptr, 0);
        if (recalls[100] < P_dest && L < L_upper_origin) {
            ...
            avg_backtrack = last_avg_backtrack;
        }
//    L_upper = strtoull(argv[8], nullptr, 0);
