...
#include "../include/efanna2e/index_nsg.202012041718.last_update_step.h"
        ...
        double &avg_iters)
{
    ...
    index.record_last_update_iter_.resize(L, 0);
...
//    std::unordered_map<unsigned, double> recalls;
    ...
    {// last update step
        for (unsigned e_i = 0; e_i < L; ++e_i) {
            index.record_last_update_iter_[e_i] /= query_num;
        }
        for (unsigned e_i = 0; e_i < L; ++e_i) {
            printf("%u\t"
                   "%f\n",
                   e_i + 1,
                   index.record_last_update_iter_[e_i]);
        }
    }
    {// Basic output
        printf(
