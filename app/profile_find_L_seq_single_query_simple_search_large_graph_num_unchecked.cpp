...
//    std::unordered_map<unsigned, double> recalls;
    ...
    {// number of unchecked
        for (unsigned iter = 0; iter < index.count_num_unchecked_.size(); ++iter) {
            printf("%u\t"
                   "%f",
                   iter,
                   index.count_num_unchecked_[iter] * 1.0 / index.count_iterations_[iter]);
            printf("\n");
        }
        index.count_num_unchecked_.clear();
