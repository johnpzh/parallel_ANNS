...
#include "../include/efanna2e/index_nsg.202011301613.num_unchecked.h"
        ...
        double &avg_iters)
{
    ...
    for (unsigned i = 0; i < query_num; i++) {
        index.Search_num_rank_0(query_load + i * dim, data_load, K, paras, res[i].data());
    }
...
//    std::unordered_map<unsigned, double> recalls;
    ...
    {// number of unchecked
        for (unsigned iter = 0; iter < index.count_is_rank_0_.size(); ++iter) {
            double prob = index.count_is_rank_0_[iter] * 1.0 / index.count_iterations_[iter];
            printf("%u\t"
                   "%f",
                   iter,
                   prob);
        ...
        index.count_is_rank_0_.clear();
        index.count_iterations_.clear();
    }
    {// Basic output
        printf(
                "L: %u "
