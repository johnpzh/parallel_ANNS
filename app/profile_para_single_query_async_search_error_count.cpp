...
//#include "../core/Searching.202008281536.error_count.h"
#include "../core/Searching.202008281536.error_count.h"

void usage(char *argv[])
{
    fprintf(stderr,
            "Usage: %s <data_file> <query_file> <nsg_path> <L> <K> <result_file> <M_max> <true_NN_file> <num_threads> <local_L> <compt_quota>\n",
            argv[0]);
}
...
//    engine.build_opt_graph();
    ...
    unsigned errors_bound = strtoull(argv[7], nullptr, 0);
    if (L < K) {
...
//    unsigned subsearch_iterations = strtoull(argv[11], nullptr, 0);
    uint64_t thread_compt_quota = strtoull(argv[11], nullptr, 0);
...
//#pragma omp parallel for
        for (unsigned q_i = 0; q_i < query_num; ++q_i) {
            engine.para_search_with_error_count_v1(
                    q_i,
                    K,
                    L,
                    set_L,
