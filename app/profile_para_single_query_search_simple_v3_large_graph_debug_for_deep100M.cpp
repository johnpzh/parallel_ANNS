//
// Created by Zhen Peng on 09/30/2020.
//
...
#include "../core/Searching.202009301055.simple_v3.large_graph.debug_for_deep100M.h"

void usage(char *argv[])
{
    fprintf(stderr,
            "Usage: %s <data_file> <query_file> <nsg_path> <L> <K> <result_file> <true_NN_file> <num_threads> <local_L> <sub_iters> <output_set_L>\n",
            argv[0]);
}
...
//                std::vector<uint8_t> is_visited(points_num, 0);
        ...
        std::ofstream fout(argv[11], std::ios_base::binary | std::ios_base::out);
        if (!fout.is_open()) {
            fprintf(stderr, "Error: cannot create file %s\n", argv[11]);
            exit(EXIT_FAILURE);
        }
        fout.write(reinterpret_cast<char *>(&query_num), sizeof(query_num)); // Write query_num
        fout.write(reinterpret_cast<char *>(&K), sizeof(K)); // Write K
...
//#pragma omp parallel for
        for (unsigned q_i = 0; q_i < query_num; ++q_i) {
            ...
            {
                unsigned i_c_start = local_queues_starts[num_threads - 1];
                unsigned i_c_bound = i_c_start + K;
                for (unsigned i_c = i_c_start; i_c < i_c_bound; ++i_c) {
                    unsigned id = set_L[i_c].id_;
                    float dist = set_L[i_c].distance_;
                    fout.write(reinterpret_cast<char *>(&id), sizeof(id));
                    fout.write(reinterpret_cast<char *>(&dist), sizeof(dist));
                }
            }
        }
