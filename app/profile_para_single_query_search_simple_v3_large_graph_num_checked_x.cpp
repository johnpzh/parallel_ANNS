...
#include "../core/Searching.202010301220.simple_v3.large_graph.num_checked.h"
...
//                std::vector<uint8_t> is_visited(points_num, 0);
                ...
                engine.count_x_checked_.resize(subsearch_iterations, 0);
                engine.count_x_merge_.resize(subsearch_iterations, 0);
...
//                        break;
//                    }
                    engine.para_search_with_simple_v3_large_graph_num_checked_every_x(
                            q_i,
                            K,
                            L,
                            set_L,
                ...
                {// Standard Deviation
                    for (unsigned inner_iter = 0; inner_iter < subsearch_iterations; ++inner_iter) {
//                        engine.stddev_every_x_[inner_iter] /= engine.count_merge_;
                        printf("%u\t%f\n",
                               inner_iter,
                               engine.count_x_checked_[inner_iter] * 1.0 / engine.count_x_merge_[inner_iter] / num_threads / L);
                    }
                }
                {// Basic output
                    printf(
