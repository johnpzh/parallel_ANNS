...
{
    fprintf(stderr,
            "Usage: %s <data_file> <query_file> <nsg_path> <L> <K> <result_file> <true_NN_file> <num_threads> <L_max>\n",
            argv[0]);
}
...
//    unsigned worker_M = value_M / num_threads;
    ...
    for (unsigned L = L_min; L <= L_max; ++L) {
        ...
        for (unsigned subsearch_iterations = 1; subsearch_iterations < L + 4; subsearch_iterations += 4) {
