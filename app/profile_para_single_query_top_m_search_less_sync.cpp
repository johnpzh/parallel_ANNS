...
{
    fprintf(stderr,
            "Usage: %s <data_file> <query_file> <nsg_path> <L> <K> <result_file> <M_max> <true_NN_file> <num_threads> <M_middle> <local_L> <full_merge_freq> <local_iter>\n",
            argv[0]);
}
...
//    unsigned local_master_queue_capacity = strtoull(argv[12], nullptr, 0);
    unsigned full_merge_freq = strtoull(argv[12], nullptr, 0);
    unsigned local_iter_bound = strtoull(argv[13], nullptr, 0);
//        int warmup_max = 1;
...
//#pragma omp parallel for
        for (unsigned q_i = 0; q_i < query_num; ++q_i) {
            engine.para_search_with_top_m_less_sync_v0(
                    M_middle,
                    value_M,
                    q_i,
                    K,
