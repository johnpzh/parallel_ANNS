...
//#include "../core/Searching.202008231534.m_width.h"
#include "../core/Searching.202008251657.m_width.v2.h"

void usage(char *argv[])
{
    fprintf(stderr,
            "Usage: %s <data_file> <query_file> <nsg_path> <L> <K> <result_file> <M_max> <true_NN_file> <num_threads> <local_L> <sub_iters> <compt_quota> <worker_M>\n",
            argv[0]);
}
...
//    engine.build_opt_graph();
    ...
    unsigned value_M = strtoull(argv[7], nullptr, 0);
    if (L < K) {
...
//    unsigned M_middle = strtoull(argv[10], nullptr, 0);
    ...
    unsigned worker_M = strtoull(argv[13], nullptr, 0);
...
//                std::vector<uint8_t> is_visited(points_num, 0);
        ...
        for (int q_i = 0; q_i < num_threads; ++q_i) {
            top_m_candidates_starts[q_i] = q_i * worker_M;
        }
        auto s = std::chrono::high_resolution_clock::now();
        engine.prepare_init_ids(init_ids, L);
//#pragma omp parallel for
        for (unsigned q_i = 0; q_i < query_num; ++q_i) {
            engine.para_search_with_top_m_m_width_v2(
                    value_M,
                    worker_M,
                    q_i,
                    K,
