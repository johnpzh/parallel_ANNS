...
{
    fprintf(stderr,
            "Usage: %s <data_file> <query_file> <nsg_path> <L> <K> <result_file> <M_max> <true_NN_file> <num_threads> <M_middle> <local_L> <local_master_L> <full_merge_freq> <compt_quota>\n",
            argv[0]);
}
...
//    omp_set_nested(1);
    ...
    uint64_t thread_compt_quota = strtoull(argv[14], nullptr, 0);
...
//#pragma omp parallel for
        for (unsigned q_i = 0; q_i < query_num; ++q_i) {
            engine.para_search_with_top_m_hierarchy_merge_v2(
                    M_middle,
                    value_M,
                    q_i,
                    K,
