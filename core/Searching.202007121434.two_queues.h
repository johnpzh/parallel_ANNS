...
{
    if (0 == queue_end) {
        queue[queue_start + queue_end++] = cand;
        return 0;
    }
    ...
    const auto insert_it = std::lower_bound(
                            ...
                            queue.begin() + queue_start + queue_end,
                            cand);
    idi insert_sub = insert_it - queue.begin() - queue_start;

    if (insert_sub != queue_end) {
        // Do we need to check duplicate here?

        if (queue_end == queue_capacity) { // Queue is full
            --queue_end;
        }
        ...
        memmove(reinterpret_cast<char *>(queue.data() + queue_start + insert_sub + 1),
                ...
                (queue_end - insert_sub) * sizeof(Neighbor));
        queue[insert_sub + queue_start] = cand;
        ++queue_end;
        return insert_sub;
    } else { // Insert at the end?
        if (queue_end < queue_capacity) { // Queue is not full
            // Insert at the end
            queue[insert_sub + queue_start] = cand;
            ++queue_end;
            return insert_sub;
...
//        is_visited[cand.id_] = 1;
        ...
        for (idi e_i = 0; e_i < out_degree; ++e_i) {
            ...
            ++tmp_distance_computation;
            distf dist = compute_distance_with_norm(v_data, query_data, v_norm);
            if (dist > worklist[worklist_start + worklist_end - 1].distance_) {
                continue;
            }
