//
// Created by Zhen Peng on 8/10/2020.
//

#include "Searching.202008090117.interval_merge.h"
        ...
        idi &last_k)
{
    ...
    for (idi c_i = c_i_start; c_i < c_i_bound && top_m_candidates_size < M; ++c_i) {
        ...
        set_L[c_i].is_checked_ = true;
        top_m_candidates[top_m_candidates_start + top_m_candidates_size++] = set_L[c_i].id_;
    }
}
 ...
 *           + interval merge
 */
void Searching::para_search_with_top_m_interval_merge_v0(
...
//        double tmp_pick_time = 0.0;
        while (count_workers_done < num_threads_) {
//        while (threads_computations_[0] < thread_compuation_quota_) {
//        while (true) {
//        while (k < L) {
            ++iter;
            ++para_iter;
...
//                       "para_iter: %u "
//                       "local_M: %u "
//                       "count_workers_done: %u",
////                       "top_m_candidates_starts: %u %u "
////                       "top_m_candidates_sizes: %u %u\n",
//                       query_id,
//                       para_iter,
//                       local_M,
//                       count_workers_done);
////                       top_m_candidates_starts[0], top_m_candidates_starts[1],
////                       top_m_candidates_sizes[0], top_m_candidates_sizes[1]);
//                printf(" t_m_c_sizes:");
//                for (int i = 0; i < num_threads_; ++i) {
//                    printf(" %u", top_m_candidates_sizes[i]);
//                }
//                printf("\n");
//            }
            count_workers_done = 0;
            time_expand_ -= WallTimer::get_time_mark();
//#pragma omp parallel for reduction(+ : tmp_count_computation, count_workers_done, tmp_pick_time)
#pragma omp parallel for reduction(+ : tmp_count_computation, count_workers_done)
...
//                uint64_t tmp_last_count_computation = tmp_count_computation;
                ...
                idi &k_uc = ks[q_i];
                idi &last_k = last_ks[q_i];
                idi &nk = nks[q_i];
                const idi local_queue_start = local_queues_starts[q_i];
                idi &local_queue_size = local_queues_sizes[q_i];
                const idi top_m_candidates_start = top_m_candidates_starts[q_i];
                idi &top_m_candidates_size = top_m_candidates_sizes[q_i];
                const idi queue_capacity = num_threads_ - 1 != q_i ? local_queue_capacity : L;
                bool is_no_unchecked = false;
//                idi count_sub_iters = 0;

                if (!top_m_candidates_size) {
                    is_no_unchecked = true;
                } else {
                    // Expand
                    nk = local_queue_size;
                    idi c_i_start = top_m_candidates_start;
                    idi c_i_bound = c_i_start + top_m_candidates_size;
                    for (idi c_i = c_i_start; c_i < c_i_bound && !is_quota_done; ++c_i) {
                        idi cand_id = top_m_candidates[c_i];
                        idi r = expand_one_candidate(
                                q_i,
                                cand_id,
                    ...
                    if (M_worker < local_M_max) {
                        M_worker <<= 1U;
                    }
                }
                // Subsearch iterations
                for (idi sub_i = 1; sub_i < subsearch_iterations && !is_quota_done; ++sub_i) {
                    // Pick
//                    tmp_pick_time -= WallTimer::get_time_mark();
//                    if (sub_i) {
...
//                    ++count_sub_iters;
                ...
                if (is_quota_done || is_no_unchecked) {
                    ++count_workers_done;
                }
//                {//test
//                    printf("query_id: %u "
...
//                               para_iter,
//                               q_i,
//                               thread_compuation_quota_,
//                               threads_computations_[q_i]);
//                    }
//                }
//                threads_computations_[q_i] += tmp_count_computation - tmp_last_count_computation;
            ...
            count_distance_computation_ += tmp_count_computation;
            tmp_count_computation = 0;
//            time_pick_top_m_ += tmp_pick_time;
//            tmp_pick_time = 0;
            time_expand_ += WallTimer::get_time_mark();
            ...
            time_full_merge_ -= WallTimer::get_time_mark();
            {
                ...
                idi r = merge_all_queues_to_master(
                        ...
                        local_queue_capacity,
                        L);
                if (r < nks[num_threads_ - 1]) {
                    nks[num_threads_ - 1] = r;
                }
                if (nks[num_threads_ - 1] <= last_ks[num_threads_ - 1]) {
                    ks[num_threads_ - 1] = nks[num_threads_ - 1];
                } else {
                    ks[num_threads_ - 1] = last_ks[num_threads_ - 1] + 1;
                }
