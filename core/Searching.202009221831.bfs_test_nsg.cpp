...
#include "Searching.202009221831.bfs_test_nsg.h"
        ...
        const idi start_id)
{
    ...
    std::vector<idi> frontiers(num_v_);
    idi frnt_end = 0;
    idi frnt_start = 0;

    frontiers[frnt_end++] = start_id;
    is_visited[start_id] = 1;

    while (frnt_start < frnt_end) {
//    for (idi v_i = 0; v_i < frnt_end; ++v_i) {
        idi cand_id = frontiers[frnt_start++];
        idi *out_edges = common_nsg_deg_ngbrs_ + common_nsg_vertex_base_[cand_id];
        idi out_degree = *out_edges++;
        // Explore cand_id
        for (idi e_i = 0; e_i < out_degree; ++e_i) {
...
{
    std::vector<uint8_t> is_visited(num_v_, 0);
    std::vector<idi> frontiers1(num_v_);
    std::vector<idi> frontiers2(num_v_);
    idi end1 = 0;
    idi end2 = 0;

    frontiers1[end1++] = start_id;
    is_visited[start_id] = 1;

    while (end1) {
        for (idi v_i = 0; v_i < end1; ++v_i) {
            idi cand_id = frontiers1[v_i];
            idi *out_edges = common_nsg_deg_ngbrs_ + common_nsg_vertex_base_[cand_id];
            idi out_degree = *out_edges++;
            // Explore cand_id
            for (idi e_i = 0; e_i < out_degree; ++e_i) {
                ...
                is_visited[nb_id] = 1;
                frontiers2[end2++] = nb_id;
            }
        }
        frontiers1.swap(frontiers2);
        end1 = end2;
        end2 = 0;
    }
