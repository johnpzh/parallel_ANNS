//
// Created by Zhen Peng on 11/11/2020.
//

#include "Searching.202011111604.simple_v3.large_graph.worst_and_best_unchecked.h"
...
void Searching::para_search_with_simple_v3_large_graph_worst_and_best(
//        const idi M,
//        const idi worker_M,
        const idi query_id,
        const idi K,
...
//                                is_quota_done);
                        ...
                        }
                        {
                            if (num_threads_ - 1 == w_i) {
                                compare_worst_and_best_unchecked(
                                        set_L,
                                        local_queues_starts,
                                        local_queues_sizes);
                            }
...
void Searching::compare_worst_and_best_unchecked(
        const std::vector<Candidate> &set_L,
        const std::vector<idi> &local_queues_starts,
        const std::vector<idi> &local_queues_sizes) // Sizes of local queue
{
    // Find the top-1's worst
    float top_1_worst = FLT_MAX;
    std::vector<idi> workers_pos(num_threads_, UINT_MAX);
    {
        for (int w_i = 0; w_i < num_threads_; ++w_i) {
            if (0 == local_queues_sizes[w_i]) {
                continue;
            }
            const idi e_i_start = local_queues_starts[w_i];
            const idi e_i_bound = e_i_start + local_queues_sizes[w_i];
            for (idi e_i = e_i_start; e_i < e_i_bound; ++e_i) {
                if (set_L[e_i].is_checked_) {
                    continue;
                }
                workers_pos[w_i] = e_i - e_i_start;
                break;
            }
//            idi pos = workers_pos[w_i] + e_i_start;
//            ++workers_pos[w_i];
            if (UINT_MAX != workers_pos[w_i]) {
                float dist = set_L[workers_pos[w_i] + e_i_start].distance_;
                ++workers_pos[w_i];
                if (FLT_MAX == top_1_worst) {
                    top_1_worst = dist;
                } else if (dist > top_1_worst) {
                    top_1_worst = dist;
                }
            }
        }
...
//        }
        ...
        float top_k_best = FLT_MAX;
        for (int w_i = 0; w_i < num_threads_; ++w_i) {
            const idi local_queue_start = local_queues_starts[w_i];
            // Find the unchecked
            if (workers_pos[w_i] >= local_queues_sizes[w_i]) {
//                printf("workers_pos[%d]: %u "
//                       "local_queues_sizes[%d]: %u "
//                       "rank_i: %u \n",
//                       w_i, workers_pos[w_i],
//                       w_i, local_queues_sizes[w_i],
//                       rank_i);
                if (workers_pos[w_i] != UINT_MAX) {
                    ++workers_pos[w_i];
                }
                continue;
            }
            idi e_i_start = local_queue_start + workers_pos[w_i];
            idi e_i_bound = local_queue_start + local_queues_sizes[w_i];
            for (idi e_i = e_i_start; e_i < e_i_bound; ++e_i) {
                ++workers_pos[w_i];
                if (set_L[e_i].is_checked_) {
                    continue;
                ...
                got_unchecked = true;
                float dist = set_L[e_i].distance_;
                if (dist < top_k_best) {
                    top_k_best = dist;
                }
//                workers_pos[w_i] = e_i - local_queue_start + 1;
                break;
            }
        }
//        {//test
//            for (int w_i = 0; w_i < num_threads_; ++w_i) {
//                if (workers_pos[w_i] < rank_i) {
//                    printf("workers_pos[%d]: %u "
//                           "rank_i: %u\n",
//                           w_i, workers_pos[w_i],
//                           rank_i);
...
//        }
        if (got_unchecked) {
            if (counts_every_rank_.size() < rank_i + 1) {
                counts_every_rank_.resize(rank_i + 1);
                counts_every_rank_[rank_i] = 0;
                points_every_rank_.resize(rank_i + 1);
                points_every_rank_[rank_i] = 0;
            }
            ++counts_every_rank_[rank_i];
            if (top_1_worst <= top_k_best) {
                ++points_every_rank_[rank_i];
            }
        } else {
            break;
        }
