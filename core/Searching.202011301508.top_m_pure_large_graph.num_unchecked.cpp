...
#include "Searching.202011301508.top_m_pure_large_graph.num_unchecked.h"
...
void Searching::para_search_with_top_m_pure_large_graph_num_checked(
        const idi M,
        const idi query_id,
        const idi K,
        const idi L,
...
//    idi M = 1;
    ...
    idi last_k;
    idi nk;
    {// for unchecked candidates.
        if (count_num_unchecked_.empty()) {
            count_num_unchecked_.resize(1, local_queues_sizes[num_threads_ - 1]);
            count_iterations_.resize(1, 1);
        } else {
            count_num_unchecked_[iter] += local_queues_sizes[num_threads_ - 1];
...
//            threads_computations_[0] += tmp_count_computation;
        ...
        {// for unchecked candidates
            idi num_unchecked = 0;
            idi e_i_start = master_queue_start;
            idi e_i_bound = e_i_start + local_queues_sizes[num_threads_ - 1];
...
void Searching::para_search_with_top_m_pure_large_graph_num_rank_0(
        const idi M,
        const idi query_id,
        const idi K,
        const idi L,
...
//    idi M = 1;
    ...
    idi nk;
    {// rank 0.
        if (0 == count_is_rank_0_.size()) {
            count_is_rank_0_.resize(1, 1);
            count_iterations_.resize(1, 1);
        } else {
            count_is_rank_0_[iter] += 1;
            ++count_iterations_[iter];
        }
    }
...
//            threads_computations_[0] += tmp_count_computation;
        ...
        {// for unchecked candidates
            if (count_is_rank_0_.size() < iter + 1) {
                count_is_rank_0_.resize(iter + 1);
                count_iterations_.resize(iter + 1);
                count_is_rank_0_[iter] = 0;
                count_iterations_[iter] = 0;
            }
            if (0 == nk) {
               ++count_is_rank_0_[iter];
            }
            ++count_iterations_[iter];
        }
    } // Iterations
