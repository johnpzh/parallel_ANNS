...
//    L3CacheMissRate cache_miss_kernel;
    ...
    std::vector<uint64_t> count_is_rank_0_;
...
//    std::vector<double> time_iterations_;
...
//            std::vector<idi> &top_m_candidates,
//            boost::dynamic_bitset<> &is_visited);
    void para_search_with_top_m_pure_large_graph_num_checked(
            const idi M,
            const idi query_id,
            const idi K,
            const idi L,
            ...
            std::vector<idi> &top_m_candidates,
            boost::dynamic_bitset<> &is_visited);
    void para_search_with_top_m_pure_large_graph_num_rank_0(
            const idi M,
            const idi query_id,
            const idi K,
            const idi L,
