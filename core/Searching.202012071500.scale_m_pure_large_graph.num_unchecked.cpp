...
#include "Searching.202012071500.scale_m_pure_large_graph.num_unchecked.h"
...
void Searching::para_search_with_top_m_merge_queues_scale_m_large_graph(
...
void Searching::para_search_with_top_m_merge_queues_scale_m_large_graph_num_unchecked(
        const idi value_M_max,
        const idi query_id,
        const idi K,
        const idi L,
...
//    std::vector<idi> top_m_candidates(M);
    ...
    idi M = 1;
    {// number of unchecked
        if (count_num_unchecked_.empty()) {
            count_num_unchecked_.resize(1, local_queues_ends[num_threads_ - 1]);
            count_iterations_.resize(1, 1);
        } else {
            count_num_unchecked_[iter] += local_queues_ends[num_threads_ - 1];
...
//                }
            ...
            {// number of unchecked
                idi num_unchecked = 0;
                idi e_i_start = base_set_L;
                idi e_i_bound = e_i_start + local_queues_ends[num_threads_ - 1];
                ...
                if (count_num_unchecked_.size() < iter + 1) {
                    count_num_unchecked_.resize(iter + 1);
                    count_iterations_.resize(iter + 1);
                    count_num_unchecked_[iter] = num_unchecked;
                    count_iterations_[iter] = 1;
                } else {
                    count_num_unchecked_[iter] += num_unchecked;
                    ++count_iterations_[iter];
                }
...
void Searching::para_search_with_top_m_merge_queues_scale_m_large_graph_last_update_step(
        const idi value_M_max,
        const idi query_id,
        const idi K,
        const idi L,
        ...
        boost::dynamic_bitset<> &is_visited)
{
    std::vector<idi> query_last_update_step(L, 0);
//    const idi base_set_L = (num_threads_ - 1) * local_queue_length;
    {
//#pragma omp parallel for
        for (idi c_i = 0; c_i < L; ++c_i) {
...
// Cannot use OpenMP here because this for-loop needs early break by the 2nd condition.
            ...
            std::vector<idi> bak_global_queue(L);
            {// last update step
                for (idi e_i = 0; e_i < L; ++e_i) {
                    bak_global_queue[e_i] = set_L[base_set_L + e_i].id_;
                }
            }
...
//                }
            ...
            {// last update step
                for (idi e_i = 0; e_i < L; ++e_i) {
                    if (bak_global_queue[e_i] != set_L[base_set_L + e_i].id_) {
                        query_last_update_step[e_i] = iter;
                    }
                }
            }
        }
...
//        is_visited.clear_all();
    ...
    {// last update step
        for (idi e_i = 0; e_i < L; ++e_i) {
            record_last_update_iter_[e_i] += query_last_update_step[e_i];
        }
    }
}
