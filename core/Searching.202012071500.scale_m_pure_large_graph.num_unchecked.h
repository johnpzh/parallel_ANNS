...
//    L3CacheMissRate cache_miss_kernel;
    ...
    std::vector<uint64_t> count_num_unchecked_;
    std::vector<uint64_t> count_iterations_;
    std::vector<double> record_last_update_iter_;
...
//            const idi value_M_middle,
    void para_search_with_top_m_merge_queues_scale_m_large_graph(
...
//        BitVector &is_visited)
    void para_search_with_top_m_merge_queues_scale_m_large_graph_num_unchecked(
            const idi value_M_max,
            const idi query_id,
            const idi K,
            const idi L,
            ...
            std::vector<idi> &top_m_candidates,
            boost::dynamic_bitset<> &is_visited);
    void para_search_with_top_m_merge_queues_scale_m_large_graph_last_update_step(
            const idi value_M_max,
            const idi query_id,
            const idi K,
            const idi L,
