//
// Created by Zhen Peng on 12/12/2020.
//

#include "Searching.202012122112.top_m_pure_large_graph.backtracking.h"
...
void Searching::para_search_with_top_m_pure_large_graph_backtracking(
        const idi M,
        const idi query_id,
        const idi K,
        const idi L,
...
//    time_initialization_ -= WallTimer::get_time_mark();
    const idi master_queue_start = local_queues_starts[num_threads_ - 1];
    const dataf *query_data = queries_load_ + query_id * dimension_;
    std::vector<uint32_t> iters(num_v_, UINT_MAX);
//    idi set_L_size;
...
//#pragma omp parallel for reduction(+ : tmp_count_computation)
        for (idi c_i = 0; c_i < top_m_candidates_size; ++c_i) {
            ...
            idi cand_id = top_m_candidates[c_i];
            {// for backtracking
//                if (!has_backtracking && (iter - 1) != iters[cand_id]) {
//                    has_backtracking = true;
//                    ++count_backtracking_;
//                }
                if ((iter - 1) != iters[cand_id]) {
                    ++count_backtracking_;
                }
                if (iters[cand_id] == UINT_MAX) {
                    printf("Impossible! Error: %s:%d\n", __FILE__, __LINE__);
                }
            }
            idi r = expand_one_candidate(
                    queue_i,
...
//            threads_computations_[0] += tmp_count_computation;
        ...
        {// update iters for backtracking
            idi e_i_start = master_queue_start;
            idi e_i_bound = e_i_start + local_queues_sizes[num_threads_ - 1];
            for (idi e_i = e_i_start; e_i < e_i_bound; ++e_i) {
                idi v_id = set_L[e_i].id_;
                if (iters[v_id] == UINT_MAX) {
                    iters[v_id] = iter;
                }
            }
        }
