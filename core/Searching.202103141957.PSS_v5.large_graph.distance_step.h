...
//private:
public:
    // For profiling a new candidate's step and distance
    struct Cand {
        uint32_t id_;
        uint32_t step_;
        double dist_;
...
//    std::vector<idi> map_new_to_old_;
    ...
    std::vector< std::vector<double> > prf_step_dist_parent_list_;
    std::vector< std::vector<idi> > cnt_step_parent_list_;
...
//            std::vector<idi> &top_m_candidates);
    void para_search_PSS_v5_large_graph_distance_step(
//        const idi M,
//        const idi worker_M,
            const idi query_id,
            const idi K,
