...
#include "Searching.202106261539.PSS_v5_LG_nn_distribution_2_steps_dist_prune.h"
...
idi Searching::expand_one_candidate_nn_distribution(
        const int worker_id,
        const idi cand_id,
        const dataf *query_data,
        const distf &dist_bound,
...
//        if (dist > dist_bound) {
        ...
        {
            if (nn_ids.find(nb_id) != nn_ids.end()) {
                // nb_id is one of the final nearest neighbors
                is_hit = true;
            }
        }
    }
...
void Searching::para_search_get_nn_distribution(
//        const idi M,
//        const idi worker_M,
        const idi query_id,
        const idi K,
        ...
        const idi subsearch_iterations,
        const std::set<idi> &nn_ids,
        std::vector< std::set<idi> > &hit_iters_list,
        idi &iter_count)
//        std::vector<idi> &top_m_candidates)
{
#ifdef BREAKDOWN_PRINT
...
//                            tmp_count_computation);
////                        is_quota_done);
                    r = expand_one_candidate_nn_distribution(
                            0,
                            cand_id,
                            query_data,
                            last_dist,
                            ...
                            is_hit);
                    {// nn distribution
                        idi current_iter = (iter - 1);
                        if (is_hit) {
                            hit_iters_list[0].insert(current_iter);
                        }
                ...
                while (worker_iter < subsearch_iterations && k_uc < local_queue_size) {
                    ...
                    if (!cand.is_checked_) {
                        ...
                        r = expand_one_candidate_nn_distribution(
                                ...
                                is_hit);
                        {// nn distribution
                            idi current_iter = (seq_iter_bound - 1) + (iter - 1 - seq_iter_bound) * subsearch_iterations + worker_iter;
                            if (is_hit) {
                                hit_iters_list[w_i].insert(current_iter);
                            }
                            iter_count = current_iter;
                        }
                        if (r <= k_uc) {
