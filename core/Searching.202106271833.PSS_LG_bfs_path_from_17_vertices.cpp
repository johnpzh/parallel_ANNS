//
// Created by Zhen Peng on 06/27/2020.
//

#include "Searching.202106271833.PSS_LG_bfs_path_from_17_vertices.h"
...
idi Searching::prepare_init_ids_only_neighbors(
        const idi selected_query_id,
        std::vector<unsigned int> &init_ids,
        const unsigned L) const
{
//    idi num_ngbrs = get_out_degree(ep_);
...
//    }
//    std::unordered_set<idi> visited_ids;
    const dataf *query_data = queries_load_ + selected_query_id * dimension_;
//    distf ep_dist = get_distance_to_query(ep_, query_data);
    boost::dynamic_bitset<> is_selected(num_v_);
    idi *out_edges = common_nsg_deg_ngbrs_ + common_nsg_vertex_base_[ep_];
//    idi *out_edges = (idi *) (opt_nsg_graph_ + ep_ * vertex_bytes_ + data_bytes_);
    idi out_degree = *out_edges++;
...
//        idi v_id = out_edges[tmp_l];
        idi v_id = out_edges[e_i];
//        distf v_dist = get_distance_to_query(v_id, query_data);
        if(is_selected[v_id]) {
            continue;
        }
//        if (v_dist > ep_dist) {
...
idi Searching::expand_one_candidate_core_subgraph(
        const int worker_id,
        const idi cand_id,
        const dataf *query_data,
        const distf &dist_bound,
...
//        boost::dynamic_bitset<> &is_visited,
        uint64_t &local_count_computation,
        std::vector<char> &is_in_core,
        const idi local_iter,
        std::vector<idi> &iter_inserted)
//        std::map<idi, distf> &edges)
//        bool &is_quota_done)
{
    uint64_t tmp_count_computation = 0;
//    _mm_prefetch(opt_nsg_graph_ + cand_id * vertex_bytes_ + data_bytes_, _MM_HINT_T0);
...
//        }
        ...
        {
            if (!is_in_core[nb_id]) {
                is_in_core[nb_id] = true;
                iter_inserted[nb_id] = local_iter;
            }
//            edges[nb_id] = dist;
        }
    }
...
void Searching::para_search_get_core_subgraph(
//        const idi M,
//        const idi worker_M,
        const idi query_id,
        const idi K,
...
//        boost::dynamic_bitset<> &is_visited,
        ...
        std::vector<char> &is_in_core,
        std::vector<idi> &iter_inserted)
//        std::vector< std::map<idi, distf> > &edgelist)
//        std::vector<idi> &top_m_candidates)
{
#ifdef BREAKDOWN_PRINT
    time_seq_ -= WallTimer::get_time_mark();
...
//            local_queue_capacity,
    ...
    {// Add into edgelist
        is_in_core[ep_] = true;
        iter_inserted[ep_] = 0;
        for (idi c_i = master_queue_start; c_i < master_queue_start + master_queue_size; ++c_i) {
            idi v_i = set_L[c_i].id_;
            is_in_core[v_i] = true;
            iter_inserted[v_i] = 1;
//            distf dist = set_L[c_i].distance_;
//            edgelist[ep_][v_i] = dist;
        }
    }
//    idi top_m_candidates_end = 0;
    const distf &last_dist = set_L[master_queue_start + master_queue_size - 1].distance_;
...
//                bool is_quota_done = false;
                ...
                while (worker_iter < subsearch_iterations && k_uc < local_queue_size) {
                    ...
                    if (!cand.is_checked_) {
                        cand.is_checked_ = true;
                        ++worker_iter;
                        idi local_iter = (iter - 1) * subsearch_iterations + worker_iter + 1;
                        cand_id = cand.id_;
//#pragma omp critical
                        {
                            r = expand_one_candidate_core_subgraph(
                                    w_i,
                                    cand_id,
                                    query_data,
                                    last_dist,
...
//
//void Searching::get_reverse_edgelist(
//        std::vector< std::vector<idi> > &r_edgelist)
//{
//    for (idi v_i = 0; v_i < num_v_; ++v_i) {
//        idi *out_edges = common_nsg_deg_ngbrs_ + common_nsg_vertex_base_[v_i];
//        idi out_degree = *out_edges++;
//
//        for (idi e_i = 0; e_i < out_degree; ++e_i) {
//            idi nb_id = out_edges[e_i];
//            r_edgelist[nb_id].push_back(v_i);
//        }
//    }
//}

/* Checke the connection */
void Searching::check_shortest_path(
        const idi dest_id,
        const std::vector< std::pair<idi, idi> > &shortest_path)
{
    idi path_length = shortest_path.size();

    for (idi v_i = 0; v_i < path_length - 1; ++v_i) {
        idi v_id = shortest_path[v_i].first;
        idi nb_id = shortest_path[v_i + 1].first;

        idi v_step = shortest_path[v_i].second;
        idi nb_step = shortest_path[v_i + 1].second;
        if (v_i) {
            printf(" [%u]:(%u, %u)", v_i + 1, nb_id, nb_step);
        } else {
            printf("[%u]:(%u, %u) [%u]:(%u, %u)", v_i, v_id, v_step, v_i + 1, nb_id, nb_step);
        }

        // Check if nb_id is in v_id's neighbor
        bool is_in = false;
        idi *out_edges = common_nsg_deg_ngbrs_ + common_nsg_vertex_base_[v_id];
        idi out_degree = *out_edges++;

        for (idi e_i = 0; e_i < out_degree; ++e_i) {
            idi e_id = out_edges[e_i];
            if (e_id == nb_id) {
                is_in = true;
                break;
            }
        ...
        if (!is_in) {
            fprintf(stderr,
                    "Error: check_shortest_path(): v_id %u does not have a nb_id %u.\n",
                    v_id, nb_id);
        }
    }
    printf("\n");
    printf("dest_id %u passed.\n", dest_id);
}

/* shortest_path: pair: <id, step> */
bool Searching::get_mononic_shortest_path(
        const idi dest_id,
        const idi selected_query_id,
        std::vector< std::pair<idi, idi> > &shortest_path) const
{
    PANNS::dataf *query_data = queries_load_ + selected_query_id * dimension_;
    idi start_id = ep_;
    std::vector<bool> is_visited(num_v_, false);
    is_visited[start_id] = true;
    std::vector<idi> queue(num_v_);
    idi queue_head = 0;
    idi queue_end = 0;
    std::vector<idi> my_parent(num_v_);
    std::vector<idi> my_step(num_v_);
    my_step[start_id] = 0;
    queue[queue_end++] = start_id;
    is_visited[start_id] = true;

    // Find the steps and parents
    bool is_found = false;
    while (!is_found && queue_end > queue_head) {
        idi v_id = queue[queue_head++];
        idi v_step = my_step[v_id];
        idi *out_edges = common_nsg_deg_ngbrs_ + common_nsg_vertex_base_[v_id];
        idi out_degree = *out_edges++;
        PANNS::dataf *v_data = data_load_ + v_id * dimension_;
        PANNS::distf v_dist = compute_distance(v_data, query_data);
        ...
        for (idi e_i = 0; e_i < out_degree; ++e_i) {
            ...
            PANNS::dataf *nb_data = data_load_ + nb_id * dimension_;
            PANNS::distf nb_dist = compute_distance(nb_data, query_data);
            is_visited[nb_id] = true;
    ...
    if (is_found) {
        idi pt_id = dest_id;
        while (pt_id != start_id) {
            idi step = my_step[pt_id];
            shortest_path.insert(shortest_path.begin(), std::make_pair(pt_id, step));
            pt_id = my_parent[pt_id];
...
/* shortest_path: pair: <id, step> */
void Searching::get_shortest_path(
        const idi dest_id,
        std::vector< std::pair<idi, idi> > &shortest_path) const
{
    idi start_id = ep_;
...
/* shortest_path: pair: <id, step>
 * vertices_on_path: pair: <step, distance> */
void Searching::get_vertices_on_path(
        const std::vector< std::vector< std::pair<idi, idi> > > &shortest_path_list,
        const idi K,
        const idi query_id,
        std::map<idi, std::pair<idi, distf> > &vertices_on_path,
        std::vector< std::set<idi> > &path_edgelist,
        std::vector<bool> &is_on_path) const
{
    const dataf *query_data = queries_load_ + query_id * dimension_;
    for (idi k_i = 0; k_i < K; ++k_i) {
        const auto &path = shortest_path_list[k_i];
//        for (const auto &vert : path) {
        for (idi v_i = 0; v_i < path.size(); ++v_i) {
            const auto &vert = path[v_i];
            idi v_id = vert.first;
            idi v_step = vert.second;
            if (v_i != path.size() - 1) {
                idi nb_id = path[v_i + 1].first;
                path_edgelist[v_id].insert(nb_id);
            }
            if (vertices_on_path.find(v_id) != vertices_on_path.end()) {
                continue;
            }
            const dataf *v_data = data_load_ + v_id * dimension_;
            distf v_dist = compute_distance(query_data, v_data);
            vertices_on_path[v_id] = std::make_pair(v_step, v_dist);
            is_on_path[v_id] = true;
        }
    }
}
...
distf Searching::get_distance_to_query(
        const idi v_id,
        const dataf *query_data) const
{
    dataf *v_data = data_load_ + v_id * dimension_;
    return compute_distance(v_data, query_data);
}

void Searching::do_bfs_to_all_monotonic(
        const idi query_id,
        std::vector<bool> &is_reached,
        std::vector<idi> &my_step,
        std::vector<distf> &my_distance,
        std::vector< std::vector<idi> > &edgelist) const
{
    const dataf *query_data = queries_load_ + query_id * dimension_;
    idi start_id = ep_;
    std::vector<bool> is_visited(num_v_, false);
...
//    std::vector<idi> my_parent(num_v_);
    my_step[start_id] = 0;
    my_distance[start_id] = get_distance_to_query(start_id, query_data);
    queue[queue_end++] = start_id;
    is_visited[start_id] = true;
    is_reached[start_id] = true;
    ...
    while (queue_end > queue_head) {
        ...
        idi out_degree = *out_edges++;
        PANNS::dataf *v_data = data_load_ + v_id * dimension_;
        PANNS::distf v_dist = my_distance[v_id];
        ...
        for (idi e_i = 0; e_i < out_degree; ++e_i) {
            ...
            is_reached[nb_id] = true;
//            my_parent[nb_id] = v_id;
            my_step[nb_id] = v_step + 1;
            my_distance[nb_id] = nb_dist;
            edgelist[v_id].push_back(nb_id);
            queue[queue_end++] = nb_id;
//            if (dest_id == nb_id) {
//                // Found
//                is_found = true;
...
void Searching::get_connected_to_nn_edgelist(
        const std::vector<bool> &is_connected_to_nn,
        const std::vector<distf> &my_dist,
        std::vector< std::vector<idi> > &connected_to_nn_edgelist) const
{
    for (idi v_id = 0; v_id < num_v_; ++v_id) {
        if (!is_connected_to_nn[v_id]) {
            continue;
        }
        distf v_dist = my_dist[v_id];
        idi *out_edges = common_nsg_deg_ngbrs_ + common_nsg_vertex_base_[v_id];
        idi out_degree = *out_edges++;
        for (idi e_i = 0; e_i < out_degree; ++e_i) {
            idi nb_id = out_edges[e_i];
            ...
            distf nb_dist = my_dist[nb_id];
            if (nb_dist > v_dist) {
                // Not monotonic
                continue;
            }
            connected_to_nn_edgelist[v_id].push_back(nb_id);
        }
    }
}

void Searching::get_reversed_edgelist(
        const idi selected_query_id,
        const std::vector<bool> &is_reached,
//        std::vector<PANNS::distf> &my_distance,
        std::vector< std::vector<PANNS::idi> > &r_edgelist) const
{
    PANNS::dataf *query_data = queries_load_ + selected_query_id * dimension_;
    for (idi v_id = 0; v_id < num_v_; ++v_id) {
        if (!is_reached[v_id]) {
...
//        my_distance[v_id] = v_dist;
        ...
        for (idi e_i = 0; e_i < out_degree; ++e_i) {
            ...
            distf nb_dist = get_distance_to_query(nb_id, query_data);
//            my_distance[nb_id] = nb_dist;
            if (nb_dist > v_dist) {
                // Not monotonic
                continue;
            }
            r_edgelist[nb_id].push_back(v_id);
        }
    }
}

void Searching::do_bfs_from_source_to_two_ends_connected(
        const std::vector< std::vector<idi> > &connected_to_nn_edgelist,
        std::vector<idi> &my_step) const
{
    std::vector<bool> is_visited(num_v_, false);
    std::vector<idi> queue(num_v_);
    idi queue_head = 0;
    ...
    while (queue_end > queue_head) {
        idi v_id = queue[queue_head++];
        idi v_step = my_step[v_id];
        const auto &edges = connected_to_nn_edgelist[v_id];
...
idi Searching::expand_one_candidate_with_parent(
        const int worker_id,
        const idi cand_id,
        const dataf *query_data,
        const distf &dist_bound,
...
//    tmp_time_pick_top_m += WallTimer::get_time_mark();
//    uint64_t tmp_last_count_computation = tmp_count_computation;
    distf cand_dist = get_distance_to_query(cand_id, query_data);
    idi nk = local_queue_capacity;

    idi cand_step = my_step[cand_id];
...
//        }
        {
            is_expanded[nb_id] = true;
            my_parent[nb_id] = cand_id;
            my_step[nb_id] = cand_step + 1;
            my_distance[nb_id] = dist;
        }
        Candidate cand(nb_id, dist, false);
        // Add to the local queue.
        idi r = add_into_queue(
...
void Searching::PSS_parallel_expanded_vertices_w_2_steps_dist_prune(
//        const idi M,
//        const idi worker_M,
        const idi query_id,
        const idi K,
...
//    idi top_m_candidates_end = 0;
    ...
    is_search_expanded[0][ep_] = true;
    my_step[ep_] = 0;
    my_distance[ep_] = get_distance_to_query(ep_, query_data);
//    my_parent[ep_] = ep_;
    for (idi v_i = master_queue_start; v_i < master_queue_start + master_queue_size; ++v_i) {
        const auto &cand = set_L[v_i];
        idi cand_id = cand.id_;
        my_step[cand_id] = 1;
        my_distance[cand_id] = get_distance_to_query(cand_id, query_data);
        my_parent[cand_id] = ep_;
        is_search_expanded[0][cand_id] = true;
    }
#ifdef BREAKDOWN_PRINT
    time_seq_ += WallTimer::get_time_mark();
#endif
...
//                            is_visited,
//                            tmp_count_computation);
                    r = expand_one_candidate_with_parent(
                            0,
                            cand_id,
                            query_data,
                            last_dist,
...
//                        }
                        ...
                        r = expand_one_candidate_with_parent(
                                ...
                                my_distance,
                                my_parent,
                                is_search_expanded[w_i]);
//                        {
//                            is_search_expanded[w_i][cand_id] = true;
//                        }
                        if (r <= k_uc) {
                            k_uc = r;
                        } else {
