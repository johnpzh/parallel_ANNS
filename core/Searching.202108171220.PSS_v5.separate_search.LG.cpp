//
// Created by Zhen Peng on 08/17/2021.
//

#include "Searching.202108171220.PSS_v5.separate_search.LG.h"
...
//    idi tmp_id = rand() % num_v_; // use tmp_id to replace rand().
...
idi Searching::merge_all_queues_to_master(
        ...
        const idi L,
        const int num_workers) const
{
    idi nk = L;
    const idi master_start = local_queues_starts[num_workers - 1];
    idi &master_size = local_queues_sizes[num_workers - 1];
    const idi w_i_bound = num_workers - 1;
...
//        const idi M,
        ...
        const int num_workers) const
{
    const idi last_queue_start = local_queues_starts[num_workers - 1];
    idi c_i_start = k_uc + last_queue_start;
    idi c_i_bound = last_queue_start + local_queues_sizes[num_workers - 1];
    idi top_m_count = 0;
    int dest_queue = 0;
...
//        }
    ...
    uint64_t tmp_count_computation = 0;
//#pragma omp parallel for num_threads(num_threads_inner_) reduction(+ : tmp_count_computation)
...
//        distf dist = compute_distance_with_norm(v_data, query_data, norm);
    ...
    cnt_dist_compt += tmp_count_computation;
...
void Searching::para_search_PSS_v5_separate_search_LG(
//        const idi M,
//        const idi worker_M,
        const idi query_id,
        const idi K,
...
//        printf("query_id: %u\n", query_id);
//    }
    const int num_workers = num_threads_;
//    const idi master_queue_start = local_queues_starts[num_workers - 1];
//    idi &master_queue_size = local_queues_sizes[num_workers - 1];
    const dataf *query_data = queries_load_ + query_id * dimension_;
#pragma omp parallel reduction(+ : cnt_dist_compt)
{
    int worker_id = omp_get_thread_num();
    boost::dynamic_bitset<> is_visited(num_v_);
    //    std::vector<PANNS::Candidate> set_L((num_workers - 1) * local_queue_capacity + L);
//    std::vector<PANNS::idi> local_queues_sizes(num_workers, 0);
    const idi local_queue_start = local_queues_starts[worker_id];
    idi &local_queue_size = local_queues_sizes[worker_id];
    std::vector<idi> init_ids(L);
    prepare_init_ids(init_ids, L);
    ...
    initialize_set_L_para(
            ...
            cnt_dist_compt);
    //    idi top_m_candidates_end = 0;
    const distf &last_dist = set_L[local_queue_start + local_queue_size - 1].distance_;
    idi iter = 0; // for debug
...
//        }
        ...
        auto &cand = set_L[local_queue_start + k];
        if (!cand.is_checked_) {
            cand.is_checked_ = true;
...
//#endif
//
//#pragma omp parallel num_threads(num_workers) reduction(+ : tmp_count_computation)
//{
//    //                bool is_quota_done = false;
//    int w_i = omp_get_thread_num();
//    const idi local_queue_start = local_queues_starts[w_i];
//    idi &local_queue_size = local_queues_sizes[w_i];
//    const idi queue_capacity = num_workers - 1 != w_i ? local_queue_capacity : L;
//    idi k_uc = num_workers - 1 != w_i ? 0 : k_master;
//    idi cand_id;
//    idi r;
//    idi worker_iter = 0;
//    while (worker_iter < subsearch_iterations && k_uc < local_queue_size) {
