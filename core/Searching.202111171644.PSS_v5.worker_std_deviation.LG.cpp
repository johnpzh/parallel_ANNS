//
// Created by Zhen Peng on 11/17/2021.
//

#include "Searching.202111171644.PSS_v5.worker_std_deviation.LG.h"
...
void Searching::para_search_PSS_v5_worker_std_deviation_LG(
//        const idi M,
//        const idi worker_M,
        const idi query_id,
        const idi K,
        ...
        std::vector<idi> &local_queues_sizes, // Sizes of local queue
        boost::dynamic_bitset<> &is_visited,
        const float std_dev_threshold)
//        const idi subsearch_iterations,
//        std::vector<double> &every_step_variance,
//        std::vector<idi> &every_step_median,
//        std::vector<idi> &merge_step)
//        std::vector<idi> &top_m_candidates)
{
...
#endif
        // Store every worker's newly inserted best candidate.
        std::vector<distf> local_best_firsts(num_threads_);
...
//        idi local_merge = 0;
        ...
        while (!no_need_to_continue) {
            ...
            ++para_iter;
//            {//test
//                printf("------- query_id: %u iter: %u local_L: %u -------\n",
//                       query_id, iter, local_queue_capacity);
//            }
            // Pick and copy top-M unchecked from Master to other workers
#ifdef DEBUG_PRINT
...
//                        }
                        ...
                        {// update local_best_firsts
                            ...
                            local_best_firsts[w_i] = set_L[local_queue_start + r].distance_;
//                            local_best_index[w_i] = r;
                        }
                        {// Check the metrics
                            if (w_i == checker_id) {
//                                ++check_count;
//                            every_step_variance.push_back(get_dist_variation(local_best_firsts));
//                                every_step_variance.push_back(get_normalized_std_dev(local_best_firsts));
//                                every_step_median.push_back(get_index_median(local_best_index));
                                if (get_normalized_std_dev(local_best_firsts) >= std_dev_threshold) {
                                    need_merge = true;
                                }
...
//                }
            ...
            // Merge
            {
                ...
                need_merge = false;
//                {// record the step
//                    if (std::find(merge_step.begin(), merge_step.end(), check_count) == merge_step.end()) {
//                        merge_step.push_back(check_count);
//                    }
//                }
//                {// count merge
//                    ++local_merge;
