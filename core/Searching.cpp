...
void Searching::search_in_batch(
        ...
        const std::vector<unsigned int> &init_ids,
        const boost::dynamic_bitset<> &is_visited,
        std::vector<std::vector<unsigned int>> &set_K_list)
{
...
//                    unsigned r = InsertIntoPool(retset.data(), L, nn); // insert location
...

    {
        for (unsigned q_i = 0; q_i < batch_size; ++q_i) {
            printf("q_i: %u\n", q_i + batch_start);
            for (size_t k_i = 0; k_i < K; ++k_i) {
                printf("id: %u dist: %f\n", set_L_list[q_i][k_i].second, set_L_list[q_i][k_i].first);
            }
        }
    }
}
...
idi Searching::insert_into_queue_john(
        ...
        PANNS::Candidate cand)
{
    ...
    while (left < right) {
        ...
        if (c_queue[mid].first < cand.first) {
            left = mid + 1;
        } else if (c_queue[mid].first > cand.first) {
            right = mid;
        } else {
            left = mid;
            break;
