//
// Created by Zhen Peng on 11/05/2020.
//

#include "Searching.202011051142.simple_v3.large_graph.stddev_metric.h"
 ...
 */
void Searching::para_search_with_simple_v3_large_graph(
//        const idi M,
//        const idi worker_M,
        const idi query_id,
        const idi K,
...
void Searching::para_search_with_simple_v3_large_graph_stddev_metric(
//        const idi M,
//        const idi worker_M,
        const idi query_id,
        const idi K,
...
void Searching::stddev_metric_top_ones(
        const idi inner_iter,
        const std::vector<Candidate> &set_L,
        const std::vector<idi> &local_queues_starts,
        const std::vector<idi> &local_queues_sizes) // Sizes of local queue
{
    // global mean of all top-first
    double global_mean;
    std::vector<double> tops(num_threads_, 0.0);
    {
        double sum = 0;
        for (int w_i = 0; w_i < num_threads_; ++w_i) {
            if (0 == local_queues_sizes[w_i]) {
                continue;
            }
            const idi local_queue_start = local_queues_starts[w_i];
            tops[w_i] = set_L[local_queue_start].distance_;
            sum += tops[w_i];
        }
        global_mean = sum / num_threads_;
    }

    double variance = 0;
    {
        for (int w_i = 0; w_i < num_threads_; ++w_i) {
            double diff = tops[w_i] - global_mean;
            variance += diff * diff;
        }
        variance /= num_threads_;
    }

    double sd = sqrt(variance);
//    stddev_every_x_[inner_iter] += sd;
    double metric = sd / tops[num_threads_ - 1];
//    stddev_metric_every_x_[inner_iter] += metric;
}

void Searching::para_search_with_simple_v3_large_graph_stddev_metric_samples(
//        const idi M,
//        const idi worker_M,
        const idi query_id,
        const idi K,
...
//        uint8_t count_workers_done = 0;
        while (true) {
            double stddev_metric_accum = 0.0;
            ++iter;
            ++para_iter;
//            {//test
//                printf("------- iter: %u -------\n", iter);
...
//                                is_quota_done);
                        ...
                        {// For standard deviation
                            if (num_threads_ - 1 == w_i) {
                                print_stddev_metric_top_ones_unchecked(
//                                        worker_iter - 1,
                                        worker_iter - 1 + (iter - 1) * subsearch_iterations,
                                        set_L,
                                        local_queues_starts,
                                        local_queues_sizes,
                                        stddev_metric_accum);
                            }
//                                print_stddev_metric_top_ones_checked(
////                                        worker_iter - 1,
//                                        worker_iter - 1 + (iter - 1) * subsearch_iterations,
//                                        set_L,
//                                        local_queues_starts,
...
void Searching::print_stddev_metric_top_ones_checked(
        const idi total_iter,
        const std::vector<Candidate> &set_L,
        const std::vector<idi> &local_queues_starts,
        const std::vector<idi> &local_queues_sizes,
        double &stddev_metric_accum) // Sizes of local queue
{
    // global mean of all top-first
    ...
    float max_dist = -FLT_MAX;
    {
        for (int w_i = 0; w_i < num_threads_; ++w_i) {
            ...
            const idi local_queue_start = local_queues_starts[w_i];
            float dist = set_L[local_queue_start].distance_;
//            {//test
//                printf("dist: %f "
//                       "id: %u\n",
//                       set_L[local_queue_start].distance_,
//                       set_L[local_queue_start].id_);
//            }
            tops[num_tops++] = dist;
    ...
    float diff_dist = max_dist - min_dist;
//    {//test
////        printf("num_tops: %u "
////               "min_dist: %f\n",
////               num_tops,
////               min_dist);
//        for (idi i_t = 0; i_t < num_tops; ++i_t) {
//            printf("tops[%u]: %f\n",
//                   i_t,
//                   tops[i_t]);
//        }
//    }
    ...
    {// Normalize and get average
        for (idi i_t = 0; i_t < num_tops; ++i_t) {
            tops[i_t] = (tops[i_t] - min_dist) / diff_dist;
//            tops[i_t] /= diff_dist;
//            tops[i_t] /= max_dist;
//            tops[i_t] /= min_dist;
            global_mean += tops[i_t];
        }
        global_mean /= num_tops;
    }
//    {//test
//        printf("num_tops: %u "
//               "min_dist: %f "
...
//           stddev_accum);
//    double metric = sd / tops[num_threads_ - 1];
//    stddev_metric_accum += metric;
//    printf("%u\t%f\t%f\n",
//           total_iter,
//           metric,
//           stddev_metric_accum);
}
void Searching::print_stddev_metric_top_ones_unchecked(
        const idi total_iter,
        const std::vector<Candidate> &set_L,
        const std::vector<idi> &local_queues_starts,
        const std::vector<idi> &local_queues_sizes,
        double &stddev_metric_accum) // Sizes of local queue
{
    ...
    float max_dist = -FLT_MAX;
    {
        for (int w_i = 0; w_i < num_threads_; ++w_i) {
            ...
            float dist;
            bool got_unchecked = false;
            const idi local_queue_start = local_queues_starts[w_i];
            const idi local_queue_bound = local_queue_start + local_queues_sizes[w_i];
            for (idi e_i = local_queue_start; e_i < local_queue_bound; ++e_i) {
                if (set_L[e_i].is_checked_) {
                    continue;
                }
                dist = set_L[e_i].distance_;
                got_unchecked = true;
                break;
            }
//            float dist = set_L[local_queue_start].distance_;
