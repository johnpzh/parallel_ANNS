#include "index_nsg.202011301613.num_unchecked.h"
...
void IndexNSG::Search(const float *query, const float *x, size_t K,
    ...
    {// for unchecked candidates.
        if (0 == count_num_unchecked_.size()) {
            count_num_unchecked_.resize(1, L);
            count_iterations_.resize(1, 1);
        } else {
            count_num_unchecked_[iter] += L;
...
//    {//test
//        printf("iter: %u\t"
//               "count_num_unchecked_.size(): %lu\t"
//               "count_iterations_.size(): %lu\n",
//               iter,
//               count_num_unchecked_.size(),
//               count_iterations_.size());
//    }
}
...
void IndexNSG::Search_num_rank_0(const float *query, const float *x, size_t K,
                      const Parameters &parameters, unsigned *indices)
{
    const unsigned L = parameters.Get<unsigned>("L_search");
    data_ = x;
