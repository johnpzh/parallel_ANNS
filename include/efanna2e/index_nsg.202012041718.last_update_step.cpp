#include "index_nsg.202012041718.last_update_step.h"
...
void IndexNSG::Search(const float *query, const float *x, size_t K,
    ...
    std::vector<unsigned> query_last_update_step(L, 0);
    ...
    while (k < (int)L) {
        ...
        if (retset[k].flag) {
            ...
            std::vector<unsigned> bak_global_queue(L);
            {// last update step
                for (unsigned e_i = 0; e_i < L; ++e_i) {
                    bak_global_queue[e_i] = retset[e_i].id;
                }
            }
            ...
            {// last update step
                for (unsigned e_i = 0; e_i < L; ++e_i) {
                    if (bak_global_queue[e_i] != retset[e_i].id) {
