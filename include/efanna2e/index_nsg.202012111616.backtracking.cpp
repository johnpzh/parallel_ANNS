#include "index_nsg.202012111616.backtracking.h"
...
void IndexNSG::Search_backtracking(const float *query, const float *x, size_t K,
                      const Parameters &parameters, unsigned *indices) {
    const unsigned L = parameters.Get<unsigned>("L_search");
    data_ = x;
    std::vector<Neighbor> retset(L + 1);
...
//    std::vector<uint32_t> parents(nd_, UINT_MAX);
//    std::vector<uint32_t> layers(nd_, UINT_MAX);
    std::vector<uint32_t> iters(nd_, UINT_MAX);
//    layers[ep_] = 0;
    iters[ep_] = 0;

    unsigned tmp_l = 0;
    for (; tmp_l < L && tmp_l < final_graph_[ep_].size(); tmp_l++) {
        uint32_t id = final_graph_[ep_][tmp_l];
        init_ids[tmp_l] = id;
        flags[init_ids[tmp_l]] = true;
