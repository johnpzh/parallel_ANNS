#include "index_nsg.202103121531.neighbor_distance_step.h"
...
//    unsigned iter = 0;
//    if (count_every_step_.size() < iter + 1) {
//        count_every_step_.push_back(0);
//        profile_step_to_avg_dist_to_query.push_back(0);
//    }
    for (unsigned i = 0; i < init_ids.size(); i++) {
        unsigned id = init_ids[i];
...
//    { // Record average distance to the query
//        double sum_dist = 0.0;
//        for (unsigned e_i = 0; e_i < L; ++e_i) {
//            sum_dist += retset[e_i].distance;
//        }
//        profile_step_to_avg_dist_to_query[iter] += sum_dist / L;
//        ++count_every_step_[iter];
//    }
...
//void IndexNSG::Search_candidate_dist_to_query(
...
//        unsigned *indices) {
//    std::vector< std::pair<double, double> > kNN_step_dist(K, std::make_pair(0.0, 0.0));
//    const unsigned L = parameters.Get<unsigned>("L_search");
//    data_ = x;
//    std::vector<Neighbor> retset(L + 1);
//    std::vector<unsigned> init_ids(L);
...
////    }
//    unsigned iter = 0;
//    const float *parent_data = data_ + dimension_ * ep_;
//    if (count_every_step_for_candidates_.size() < iter + 1) {
//        count_every_step_for_candidates_.resize(iter + 1);
//        count_every_step_for_candidates_[iter] = 0;
//        count_every_step_for_neighbors_.resize(iter + 1);
//        count_every_step_for_neighbors_[iter] = 0;
//        profile_step_to_avg_dist_to_query_.resize(iter + 1);
//        profile_step_to_avg_dist_to_query_[iter] = 0;
//        profile_step_to_dist_to_parent_.resize(iter + 1);
//        profile_step_to_dist_to_parent_[iter] = 0;
//    }
//    for (unsigned i = 0; i < init_ids.size(); i++) {
//        unsigned id = init_ids[i];
//        ++count_distance_computation_;
...
//        retset[i] = Neighbor(id, dist, true);
//        // flags[id] = true;
//        { // Record distance between the neighbor and its parent
//            if (i < final_graph_[ep_].size()) {
//                const float *child_data = data_ + dimension_ * id;
//                float dst = distance_->compare(child_data, parent_data, (unsigned)dimension_);
//                profile_step_to_dist_to_parent_[iter] += dst;
//                ++count_every_step_for_neighbors_[iter];
//            }
//        }
//        {// kNN step and distance
//            if (i < K) {
//                kNN_step_dist[i].second = dist;
//            }
//        }
//    }
//    { // Record average distance to the query
...
//            sum_dist += retset[e_i].distance;
//        }
//        profile_step_to_avg_dist_to_query_[iter] += sum_dist / L;
//        ++count_every_step_for_candidates_[iter];
//    }
//
//    std::sort(retset.begin(), retset.begin() + L);
//    int k = 0;
...
//            {// Prepare data
//                ++iter;
//                parent_data = data_ + dimension_ * n;
//                if (count_every_step_for_candidates_.size() < iter + 1) {
//                    count_every_step_for_candidates_.resize(iter + 1);
//                    count_every_step_for_candidates_[iter] = 0;
//                    count_every_step_for_neighbors_.resize(iter + 1);
...
//                {// kNN
//                    if (r < (int)K) {
//                        for (unsigned e_i = r; e_i < K; ++e_i) {
//                            kNN_step_dist[e_i].first = iter;
//                            kNN_step_dist[e_i].second = retset[e_i].distance;
//                        }
//                    }
//                }
//            }
...
//            set_kNN_step_dist_[e_i].first += kNN_step_dist[e_i].first;
//            set_kNN_step_dist_[e_i].second += kNN_step_dist[e_i].second;
//        }
//    }
//}

void IndexNSG::Search_cand_point_step_dist(
        const unsigned query_id,
        const float *query,
        const float *x,
        size_t K,
        ...
        unsigned *indices) {
//    std::vector< std::pair<double, double> > kNN_step_dist(K, std::make_pair(0.0, 0.0));
//    std::vector< std::pair<uint32_t, double> > kNN_step_dist(K, std::make_pair(0.0, 0.0));
//    std::vector< std::pair<uint32_t, double> > cand_step_dist_to_query;
//    std::vector< std::pair<uint32_t, double> > cand_step_dist_to_parent;
struct Cand {
    uint32_t id_;
    uint32_t step_;
    double dist_;
    ...
    Cand() = default;
    Cand(uint32_t id, uint32_t step, double dist) : id_(id), step_(step), dist_(dist) { }
};
//    std::vector< Cand > kNN_step_dist(K, Cand(0, 0, 0.0));
    std::vector< Cand > cand_step_dist_to_query;
    std::vector< Cand > cand_step_dist_to_parent;
...
//    }
    ...
    while (k < (int)L) {
        ...
        if (retset[k].flag) {
            ...
            bool is_added = false;
            const float *parent_data = data_ + dimension_ * n;
            {// Prepare data
                ++iter;
//                if (count_every_step_for_candidates_.size() < iter + 1) {
//                    count_every_step_for_candidates_.resize(iter + 1);
...
//                    if (r < (int)K) {
//                        for (unsigned e_i = r; e_i < K; ++e_i) {
//                            kNN_step_dist[e_i].id_ = id;
//                            kNN_step_dist[e_i].step_ = iter;
//                            kNN_step_dist[e_i].dist_ = retset[e_i].distance;
//                        }
//                    }
//                }
                {// New candidates' step and distance
                    cand_step_dist_to_query.emplace_back(id, iter, dist);
                    const float *child_data = data_ + dimension_ * id;
                    float dst = distance_->compare(child_data, parent_data, (unsigned)dimension_);
                    cand_step_dist_to_parent.emplace_back(id, iter, dst);
                }
            }
//            { // Record average distance to the query
//                if (is_added) {
...
//    }
    {// Print
        printf("Step\tCand_to_Query\tCand_to_parent\t100-NN_to_query\n");
        // New candidate's step and distance to query and parent
        for (uint32_t e_i = 0; e_i < cand_step_dist_to_query.size(); ++e_i) {
            printf("%u\t"
                   "%f\t"
                   "%f\n",
                   cand_step_dist_to_query[e_i].step_,
                   cand_step_dist_to_query[e_i].dist_,
                   cand_step_dist_to_parent[e_i].dist_);
        }

        // Final kNN's step and distance to query
//        for (const auto &e : kNN_step_dist) {
        for (uint32_t v_i = 0; v_i < K; ++v_i) {
            uint32_t id = retset[v_i].id;

            // Find it
            for (const auto &c : cand_step_dist_to_query) {
                if (c.id_ != id) {
                    continue;
                }
                printf("%u\t\t\t"
