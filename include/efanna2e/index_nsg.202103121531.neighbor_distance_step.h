...
//    std::vector<uint32_t> count_every_step_;
    std::vector<double> profile_step_to_dist_to_parent_;
    std::vector<uint32_t> count_every_step_for_neighbors_;
    std::vector<double> profile_step_to_avg_dist_to_query_;
    std::vector<uint32_t> count_every_step_for_candidates_;
    std::vector< std::pair<double, double> > set_kNN_step_dist_;
//    std::vector< std::pair<uint32_t, double> > set_cands_step_dist_to_query_;
//    std::vector< std::pair<uint32_t, double> > set_cands_step_dist_to_parent_;
...
//    void Search_candidate_dist_to_query(
...
//            const Parameters &parameters,
//            unsigned *indices);
    void Search_cand_point_step_dist(
            const unsigned query_id,
            const float *query,
            const float *x,
            size_t k,
