#include "index_nsg.202104211705.findroot.h"
...
// Changed to speedup but not necessarily correct
// One run then all isolated vertices should be linked
void IndexNSG::findroot(
        boost::dynamic_bitset<> &flag,
//        unsigned &root,
        unsigned &unlinked_cnt,
        const Parameters &parameter)
{
#pragma omp parallel for
    for (unsigned un_i = 0; un_i < nd_; ++un_i) {
        if (flag[un_i]) {
            continue;
        }
        flag[un_i] = true;
...
//        findroot(flags, root, parameter);
    ...
    {//test
        ...
        boost::dynamic_bitset<> is_linked(nd_, 0);
        DFS(is_linked, ep_, linked_cnt);
        printf("linked_cnt: %u\n", linked_cnt);
    }
    for (size_t i = 0; i < nd_; ++i) {
