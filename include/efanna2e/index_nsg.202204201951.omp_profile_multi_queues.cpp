#include "index_nsg.202204201951.omp_profile_multi_queues.h"
...
void IndexNSG::Search(const float *query, const float *x, size_t K,
    ...
    std::sort(retset.begin(), retset.begin() + L);
    std::vector< std::vector< std::pair<unsigned, float> > > local_queues(num_threads_);
//    {//test
...
//    }
    ...
    while (k < (int)L) {
        ...
        if (retset[k].flag) {
            ...
            uint64_t tmp_count_compt = 0;
#pragma omp parallel for reduction(+ : tmp_count_compt)
            for (unsigned m = 0; m < final_graph_[n].size(); ++m) {
                int worker_id = omp_get_thread_num();
                auto &queue = local_queues[worker_id];
                unsigned id = final_graph_[n][m];
                if (flags[id]) continue;
                flags[id] = 1;
//                ++count_distance_computation_;
...
//                }
            ...
            // Merge
            {
                ...
                for (int worker_id = 0; worker_id < num_threads_; ++worker_id) {
                    auto &queue = local_queues[worker_id];
                    for (auto &node : queue) {
                        Neighbor nn(node.first, node.second, true);
                        int r = InsertIntoPool(retset.data(), L, nn);
                        if (r < nk) {
                            nk = r;
                        }
