...
if len(sys.argv) != 6:
    print(f'Usage: {sys.argv[0]} <input_file1> <input_file2> <input_file3> <input_file4> <output_curve_file>')
    exit()
with open(sys.argv[1]) as fin_1, \
        open(sys.argv[2]) as fin_2, \
        open(sys.argv[3]) as fin_3, \
        open(sys.argv[4]) as fin_4, \
        open(sys.argv[5], 'w') as fout:
    # columns = []

    # is_first = False
    # Read accuracy and query-per-second and write
    # last_acc = ""
    for line_1, line_2, line_3, line_4 in zip(fin_1, fin_2, fin_3, fin_4):
        line_1 = line_1.strip()
        line_2 = line_2.strip()
        line_3 = line_3.strip()
        line_4 = line_4.strip()
        fout.write(line_1 + "\t" + line_2 + "\t" + line_3 + "\t" + line_4 + "\n")
        # if is_first:
        #     is_first = False
        #     continue
