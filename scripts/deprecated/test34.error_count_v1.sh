...
bin="./profile_para_single_query_async_search_error_count"
...
:> output.${label}.raw.txt
for ((EB = 1; EB <= 8; ++EB)); do
    for ((lol = 200; lol >= 30; lol -= 20)); do
        ${bin} ${data_dir}/sift1m/sift_base.fvecs ${data_dir}/sift1m/sift_query.fvecs ${data_dir}/sift1m/sift.nsg 200 200 output.ivecs ${EB} ${data_dir}/sift1m/sift.true-100_NN.q-10000.binary 8 ${lol} 200000000 | tee -a output.${label}.raw.txt
    done
done
