...
if len(sys.argv) != 12:
    print(f"{sys.argv[0]} <app> <data_dir> <data> <tag> <num_t> <L_low> <L_up> <L_step> <X_low> <X_up> <X_step>")
...
L_step = int(sys.argv[8])
X_lower = int(sys.argv[9])
X_upper = int(sys.argv[10])
X_step = int(sys.argv[11])
...
subprocess.run(F'python3 ../scripts/output_rows_to_table.py {raw_file} {rows_file} 2 3 10 9 12 13 1 15 16', shell=True, check=True)
