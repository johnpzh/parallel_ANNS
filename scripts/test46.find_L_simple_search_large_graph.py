...
P_level = 0.9
# L_upper = 2000
for L_upper in [16000, 32000, 64000, 128000, 256000, 512000]:
    command = F"{bin} {data_dir}/{data_name}_base.fvecs {data_dir}/{data_name}_query.fvecs {data_dir}/{data_name}.nsg " \
              F"{L_lower} 100 output.ivecs {data_dir}/{data_name}.true-100_NN.q-10000.binary {L_upper} {P_level} " \
              F"| tee -a {raw_file}"
...
subprocess.run(F"python3 ../scripts/output_format.py {rows_file} {table_file} 0:6")
