...
if len(sys.argv) < 9:
    print(f"{sys.argv[0]} <app> <data_dir> <data> <tag> <L_low> <L_up> <R> <P_target> [<P_target> ...]")
...
command = F"{bin} {data_dir}/{data_name}_base.fvecs {data_dir}/{data_name}_query.fvecs {data_dir}/{data_name}_R{R}.nsg " \
          F"{L_lower} 100 output.ivecs {data_dir}/{data_name}.true-100_NN.v2.binary " \
          F"1 9999999 {L_upper} {P_level} " \
          F"| tee -a {raw_file}"
