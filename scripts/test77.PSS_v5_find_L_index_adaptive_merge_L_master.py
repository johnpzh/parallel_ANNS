...
if len(sys.argv) < 11:
    print(f"{sys.argv[0]} <app> <data_dir> <data> "
          f"<tag> <num_t> <L_master> <L_local_low> "
          f"<L_local_up> <index_threshold> <P_target> [<P_target> ...]")
...
L_local_lower = int(sys.argv[7])
L_local_upper = int(sys.argv[8])
index_threshold = float(sys.argv[9])
...
command = F"{bin} {data_dir}/{data_name}_base.fvecs {data_dir}/{data_name}_query.fvecs {data_dir}/{data_name}.nsg " \
          ...
          F"{L_master} {L_local_lower} {L_local_upper} {index_threshold} " \
          F"{P_level} " \
          F"| tee -a {raw_file}"
