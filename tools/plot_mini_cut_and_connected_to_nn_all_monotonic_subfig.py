...
# color count: [blue, red, grey, purple]
def add_to_step_color(
        step_color_count: dict,
        v_step: int,
        v_color: str):
    if v_color not in ["tab:blue", "tab:red", "tab:grey", "tab:purple"]:
        raise ValueError(f"v_color {v_color} is not supported.")

    if v_step not in step_color_count:
        step_color_count[v_step] = [0, 0, 0, 0]
    ...
    elif v_color == "tab:purple":
        step_color_count[v_step][3] += 1
...
def plot(args):
    ...
    mini_cut_file = args.mini_cut_txt
    ...
    with open(input_file, "r") as fin,\
        open(mini_cut_file, "r") as fin_cut:

        ## Read mini cut
        num_cut_vertices = int(fin_cut.readline())
        cut_vertices = list()
        for _ in range(num_cut_vertices):
            cut_vertices.append(int(fin_cut.readline()))
        num_cut_edges = int(fin_cut.readline())
        cut_edges = list()
        for _ in range(num_cut_edges):
            s, e = [int(x) for x in fin_cut.readline().split()]
            cut_edges.append((s, e))
            # #### test
            # print(f"cut: {s} {e}")
            # #### end test
        ...
        for p_i in range(num_p):
            ...
            if v_id in cut_vertices:
                purple_x.append(v_step)
                purple_y.append(v_dist)
                add_to_step_color(step_color_count, v_step, "tab:purple")
            elif v_id in knn:
                found_knn.add(v_id)
                red_x.append(v_step)
                red_y.append(v_dist)
        ...
        # Plot points
        ax.scatter(purple_x, purple_y, c="tab:purple", label="cut", alpha=0.6)
        ...
        for step in range(max_step + 1):
            ...
            cnt_purple = step_color_count[step][3]
            ax.text(x, y, f"B:{cnt_blue}\nR:{cnt_red}\nGy:{cnt_grey}\nPp:{cnt_purple}", fontsize="xx-small", ha="center")
            print(f"Step: {step} Blue: {cnt_blue} Red: {cnt_red} Grey: {cnt_grey} Purple: {cnt_purple}")
        ...
        for a_i in range(num_arr):
            ...
            if (sid, eid) in cut_edges:
                color = "tab:purple"
                # #### test
                # print(f"!! hit cut: {sid} {eid}")
                # #### end test
...
def get_args():
    ...
    parser.add_argument("mini_cut_txt", help="input mini cut text file")
